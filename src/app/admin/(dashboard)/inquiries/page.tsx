...
export default async function InquiriesPage({
  ...
  return (
    <div>
      ...
      <form className="mb-6 mt-6 flex flex-wrap gap-3" action="/admin/inquiries" method="get">
        <input
          type="text"
          name="q"
          defaultValue={params.q}
          placeholder="Search by email or description…"
          className="min-w-[240px] flex-1 rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm placeholder:text-slate-400 focus:border-slate-500 focus:outline-none"
        />
        <select
          name="status"
          defaultValue={statusFilter ?? ""}
          className="rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm focus:border-slate-500 focus:outline-none"
        >
